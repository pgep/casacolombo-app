...
class Produto {
  static async initTable() {
    try {
      // 1. Criação da tabela (SEM o campo imagem, COM imagem_id)
      const query = `
      ...
      // 2. 🗑️ REMOVER campo imagem ANTIGO (se existir)
      try {
        await db.query(`
  ...
  static async update(id, data) {
    ...
    const query = `
      ...
      SET nome = $1, descricao = $2, tipo_produto_id = $3, 
          imagem_id = $4, custo_total = $5, preco_venda = $6, 
          preco_final = $7, ativo = $8, updated_at = CURRENT_TIMESTAMP
      WHERE id = $9
      RETURNING *
    `;
